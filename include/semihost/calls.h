...
uintptr_t sys_semihost_clock(void);

int sys_semihost_close(int fd);

uint64_t sys_semihost_elapsed(void);

int sys_semihost_errno(void);

__noreturn void sys_semihost_exit(uintptr_t exception, uintptr_t subcode);

__noreturn void sys_semihost_exit_extended(uintptr_t code);
...
int sys_semihost_remove(const char *pathname);
...
uintptr_t sys_semihost_tickfreq(void);
