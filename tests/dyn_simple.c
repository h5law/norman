...
int main(int argc, char *argv[])
{
    ...
    //        vec.end_ptr == vec2.end_ptr && vec.load_factor ==
    //        vec2.load_factor);
    ...
    //
    norm_map_t map = {0};
    map = norm_map_init(20, 0.7, sizeof(uint64_t), *default_hasher);
    uint64_t p = 20;
    ...
    assert(map.table.capacity == 32);

    char k[16] = {0};
    for (int i = 0; i < map.table.capacity; ++i) {
        norm_map_entry_t *entry =
                (void *)(((char *)(map.table.array)) +
                         (i * (sizeof(norm_map_entry_t) + NORM_MAP_MAX_KEY_LEN +
                               sizeof(uint64_t))));
        snprintf(k, 16, "%s", (char *)entry->kv);
        uint64_t *val = (void *)(((char *)(entry->kv)) + NORM_MAP_MAX_KEY_LEN);
        uint64_t *got =
                norm_map_get(&map, k, entry->key_length, sizeof(uint64_t));
        printf("%d -> %llu  \t(%s :: %lld)\n", i, *val, k, *got);
    }
